...
const SFX = (() => {
  let ctx = null;
  let muted = false;
  try { muted = localStorage.getItem('kowloon.muted') === '1'; } catch (_) { /* 저장소 차단 */ }
  ...
  return {
    ...
    setMuted(v) {
      muted = !!v;
      try { localStorage.setItem('kowloon.muted', muted ? '1' : '0'); } catch (_) { /* 무시 */ }
    },
    ...
    // ── 구룡: 누아르 소리
    stamp: () => { noise({ dur: 0.18, vol: 0.9, freq: 500, decay: 5 }); tone({ f: 90, f2: 40, dur: 0.2, vol: 0.4 }); },
    bullet: () => { tone({ f: 2400, f2: 1800, type: 'square', dur: 0.03, vol: 0.05 }); noise({ dur: 0.06, vol: 0.3, freq: 4200, type: 'highpass', delay: 0.02 }); tone({ f: 1400, f2: 900, type: 'triangle', dur: 0.12, vol: 0.05, delay: 0.05 }); },
    type: () => { for (let i = 0; i < 5; i++) noise({ dur: 0.03, vol: 0.18, freq: 2600, type: 'highpass', delay: i * 0.07 + Math.random() * 0.03 }); },
    siren: () => { for (let i = 0; i < 4; i++) { tone({ f: 740, type: 'triangle', dur: 0.32, vol: 0.05, delay: i * 0.64 }); tone({ f: 988, type: 'triangle', dur: 0.32, vol: 0.05, delay: i * 0.64 + 0.32 }); } },
    cuffs: () => { tone({ f: 3200, f2: 2600, type: 'square', dur: 0.05, vol: 0.05 }); noise({ dur: 0.1, vol: 0.4, freq: 5000, type: 'highpass', delay: 0.02 }); tone({ f: 2800, type: 'triangle', dur: 0.18, vol: 0.05, delay: 0.1 }); },
    thunder: () => { noise({ dur: 1.8, vol: 0.5, freq: 180, decay: 1.5 }); noise({ dur: 0.3, vol: 0.4, freq: 900, decay: 3, delay: 0.05 }); },
    rain: () => { noise({ dur: 3.5, vol: 0.06, freq: 5000, type: 'highpass', decay: 0.3 }); },
    heart: () => { tone({ f: 70, f2: 50, dur: 0.14, vol: 0.4 }); tone({ f: 70, f2: 50, dur: 0.14, vol: 0.32, delay: 0.24 }); },
    wrong: () => { tone({ f: 220, f2: 160, type: 'sawtooth', dur: 0.4, vol: 0.06 }); tone({ f: 233, f2: 170, type: 'sawtooth', dur: 0.4, vol: 0.05 }); },
    right: () => { [523, 784, 1047].forEach((f, i) => tone({ f, type: 'triangle', dur: 0.3, vol: 0.08, delay: i * 0.1 })); noise({ dur: 0.2, vol: 0.5, freq: 600, decay: 5, delay: 0.3 }); },
    scream: () => { tone({ f: 900, f2: 1400, type: 'sawtooth', dur: 0.5, vol: 0.04 }); tone({ f: 1400, f2: 600, type: 'sawtooth', dur: 0.6, vol: 0.04, delay: 0.45 }); },
    unlock: () => ac(),
  };
})();

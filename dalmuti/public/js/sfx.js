...
const SFX = (() => {
  let ctx = null;
  let muted = false;
  try { muted = localStorage.getItem('dalmuti.muted') === '1'; } catch (_) { /* 저장소 차단 */ }
  ...
  return {
    ...
    setMuted(v) {
      muted = !!v;
      try { localStorage.setItem('dalmuti.muted', muted ? '1' : '0'); } catch (_) { /* 무시 */ }
    },
    ...
    fanfare: () => [392, 523, 659, 784, 1047].forEach((f, i) => { tone({ f, type: 'sawtooth', dur: 0.28, vol: 0.05, delay: i * 0.12 }); tone({ f: f / 2, type: 'triangle', dur: 0.3, vol: 0.06, delay: i * 0.12 }); }),
    coin: () => { tone({ f: 1320, f2: 1760, type: 'triangle', dur: 0.12, vol: 0.06 }); tone({ f: 1760, type: 'triangle', dur: 0.2, vol: 0.05, delay: 0.08 }); },
    pass: () => tone({ f: 330, f2: 260, type: 'triangle', dur: 0.14, vol: 0.05 }),
    slam: () => { noise({ dur: 0.14, vol: 0.5, freq: 900, decay: 4 }); tone({ f: 140, f2: 70, dur: 0.16, vol: 0.2 }); },
    riot: () => { for (let i = 0; i < 6; i++) noise({ dur: 0.3, vol: 0.18, freq: 700 + Math.random() * 600, delay: i * 0.12 }); tone({ f: 196, f2: 150, type: 'sawtooth', dur: 0.8, vol: 0.05 }); },
    bells: () => [1047, 1319, 1568, 2093].forEach((f, i) => tone({ f, type: 'triangle', dur: 0.4, vol: 0.05, delay: i * 0.09 })),
    unlock: () => ac(),
  };
})();
